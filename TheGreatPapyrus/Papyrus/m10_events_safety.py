...
class Battle:
    ...
    def make_embed(self):
        """Compact battle UI - HP at top so buttons stay near HP (less scrolling)."""
        ...
        boss_bar = hp_bar(boss_hp, boss_max, length=8)
        player_bar = hp_bar(player_hp, player_max, length=8)
        ...
        log_lines = self.log[-3:] if self.log else ["…"]
        cleaned = []
        for x in log_lines:
            s = str(x).replace(chr(10), " ").strip()
            cleaned.append(s[:55])
        log_text = " · ".join(cleaned)
        if len(log_text) > 130:
            log_text = log_text[:127] + "…"

        you = label_for_member(self.player)
        nl = chr(10)
        desc = (
            f"**👹 Boss** {threat} `{boss_pct}%`" + nl
            + f"{boss_bar} **{boss_hp:,}**/**{boss_max:,}**  ⚔`{int(self.boss['attack']):,}` 🛡`{int(self.boss['defense']):,}`" + nl
            + f"**❤️ {you}** `{player_pct}%`" + nl
            + f"{player_bar} **{player_hp:,}**/**{player_max:,}**  ⚔`{atk:,}` 🛡`{deff:,}`" + nl
            + f"_{log_text}_"
        )

        embed = discord.Embed(
            title=("💀 " if is_final else "⚔️ ") + str(self.boss["name"])[:80],
            description=desc[:4000],
            color=theme,
        )
        embed.set_author(name="FINAL" if is_final else "BATTLE")

        # Thumbnail for PNG and GIF (both display). Full set_image made mobile users scroll.
        try:
            url = ""
            try:
