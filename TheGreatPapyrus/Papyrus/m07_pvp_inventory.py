...
def build_inventory_embed(guild, member):
    """Inventory panel - stats + currently equipped only (lists live in buttons)."""
    guild_id = guild.id
    user_id = member.id
    ...
    embed = discord.Embed(
        ...
        description=(
            f"{thick}\n"
            f"**LV {player['level']}**  -  {bar}  `{int(player['hp'] or 0):,}` / `{max_hp_total:,}`\n"
            f"⚔️ **{atk:,}** ATK   -   🛡️ **{deff:,}** DEF   -   💰 **{player['gold']:,}** G\n"
            f"✨ `{int(player['xp'] or 0):,}` / `{xp_need:,}` XP\n"
            f"{rule}"
        ),
        color=discord.Color.from_str("#5DADE2"),
    )
    avatar_url = get_player_avatar_url(guild_id, member.id, member)
    author_name = "INVENTORY"
    try:
        if is_bot_creator(member.id):
            author_name = f"{CREATOR_TAG} INVENTORY"
        elif player_has_admin_tag(member):
            author_name = f"{ADMIN_TAG} INVENTORY"
    except Exception:
        pass
    if avatar_url:
        try:
        ...
        # GIFs animate best as embed image; static can be thumbnail
        apply_embed_media(embed, avatar_url, prefer_image=is_gif_url(avatar_url))
    else:
        embed.set_author(name=author_name)

    # Prefer equipped weapon/armor/soul custom image under the panel
    try:
    ...
    embed.add_field(name="⚔️  Weapon", value=weapon_line[:1024], inline=False)
    embed.add_field(name="🛡️  Armor", value=armor_line[:1024], inline=False)
    embed.add_field(name="👻  Soul", value=soul_line[:1024], inline=False)
    try:
        prog = player_progression_summary(guild_id, user_id)
        if prog:
            embed.add_field(name="✨  Rebirth / Ascend", value=prog[:1024], inline=False)
    except Exception as e:
        print("inventory progression:", e)
    embed.add_field(name="🔥  Abilities", value="\n".join(slot_lines)[:1024], inline=False)
    embed.add_field(name="🎭  Boss role", value=boss_role_line[:1024], inline=False)
    # Always show rebirth shard balance (even 0)
    try:
    ...
    embed.add_field(
        name="💎  Rebirth Shards",
        value=f"**{shard_n:,}** - survives Rebirth - shop/craft currency",
        inline=False,
    )
    embed.add_field(
        name="🌟  Ascended Shards",
        value=f"**{ashard_n:,}** - survives Ascend - shop/player-shop currency",
        inline=False,
    )
    embed.add_field(name="🎒  Items", value=item_summary[:1024], inline=False)
    try:
        ...
        pass
    embed.set_footer(text="◀ ▶ switch pages - Home - Shop - Player Shop - Craft - Combat - Progress - Profile")
    return embed


async def return_to_inventory_ui(interaction, owner, guild_id, notice: str = None):
    """Edit the current message back to the same inventory panel layout."""
...
async def edit_inventory_subpanel(interaction, owner, guild_id, *, content=None, embed=None, view=None):
    """Edit the inventory message in-place and attach Back to inventory."""
    try:
        if view is not None:
    ...
    async def _goto(self, interaction, page):
        ...
        try:
            ...
            blurbs = {
                ...
                3: "🔨 **Craft** — recipes",
                4: "⚔️ **Combat** — party · rush · PvP · clear",
                5: "✨ **Progress** — rebirth · ascend · codes",
                6: "☠️ **Kills** — boss stats & kill roles",
                7: "🎭 **Profile** — custom name & pfp",
            }
            try:
    ...
    async def _goto(self, interaction: discord.Interaction, new_page: int):
        ...
        try:
            embed, page, pages = await build_leaderboard_embed(
                guild, new_page, viewer_id=self.viewer_id
            )
            view = LeaderboardView(self.guild_id, page, pages, self.viewer_id)
            await interaction.edit_original_response(embed=embed, view=view)
        except Exception as e:
            try:
                await interaction.edit_original_response(
                    content=f"❌ Could not load page: {e}",
                    embed=None,
                    view=None,
                )
            except Exception:
