...
def _econ_bal_embed(guild_id, user):
    ...
    emb = discord.Embed(
        title=f"{em} Wallet — {getattr(user, 'display_name', user)}",
        description=(
            f"**Cash:** `{int(w['cash'] or 0):,}` {name}\n"
            f"**Bank:** `{int(w['bank'] or 0):,}` {name}\n"
            f"**Total:** `{int(w['cash'] or 0) + int(w['bank'] or 0):,}`\n\n"
            f"Earned lifetime: `{int(w['total_earned'] or 0):,}` · Lost: `{int(w['total_lost'] or 0):,}`\n"
            f"Crimes OK/Fail: `{int(w['crimes_ok'] or 0)}`/`{int(w['crimes_fail'] or 0)}` · Robs: `{int(w['robs_ok'] or 0)}`"
        ),
...
async def econ_cmd(
    ...
    if act == "help":
        await interaction.response.send_message(
            embed=discord.Embed(
                ...
                description=(
                    "`/econ balance` · `daily` · `work` · `crime` · `rob @user`\n"
                    "`deposit/withdraw amount` · `pay @user amount`\n"
                    "`coinflip/slots/dice amount` · `lottery` (buy ticket)\n"
                    "`shop` · `leaderboard`\n"
                    "`convert-to-gold amount` · `convert-to-shards amount`\n\n"
                    "Wins can convert into **RPG gold / shards** or buy gear in the economy shop.\n"
                    f"Currency: **{cname}** {em}"
                ),
    ...
    if act == "shop":
        ...
        emb = discord.Embed(title=f"{em} Economy Shop", description="\n".join(lines), color=discord.Color.dark_green())
    ...
    if act == "lb":
        ...
        for i, r in enumerate(rows, 1):
            lines.append(f"**{i}.** <@{r['user_id']}> — `{int(r['total'] or 0):,}` {cname}")
        await interaction.response.send_message(
            embed=discord.Embed(
                title=f"{em} Leaderboard",
