...
def build_portal_embed(guild, level, boss, player=None):
    """Portal preview - compact full-width text, boss art as thumbnail (not stretched columns)."""
    ...
    stats_line = (
        f"❤️ `{int(boss['hp']):,}`   "
        f"⚔️ `{int(boss['attack']):,}`   "
        f"🛡️ `{int(boss['defense']):,}`   "
        f"⭐ `{int(boss['xp'] or 0):,}` XP   "
        f"💰 `{int(boss['gold'] or 0):,}` G"
    )
    ...
    if is_uf:
        embed = discord.Embed(
            ...
            description=(
                ...
                "```\n"
                "╔══════════════════════════════════════╗\n"
                "║   A P E X   O F   T H E   U N I V   ║\n"
                "║      beyond final · beyond hope     ║\n"
                "╚══════════════════════════════════════╝\n"
                "```\n"
                f"▸ {stats_line}\n\n"
                "**ENTER UNIVERSE FINAL** · **SKIP** · **MENU**\n"
                "*the strings of an entire world are watching.*"
            ),
            color=theme,
        )
        embed.set_author(name=f"{player_name}'s UNIVERSE APEX" if player_name else "🌌 UNIVERSE APEX")
        if boss_img:
            apply_embed_media(embed, boss_img)
        fill_boss_info_embed(embed, guild, boss, compact=True)
        foot = f"🌌 UNIVERSE FINAL · {level_name} · beyond final"
        if player_name:
    ...
    if is_final:
        embed = discord.Embed(
            title=f"💀 FINAL - {boss['name']}",
            description=(
                uni_line
                + f"**{level_emoji} {level_name}**"
                + (f" - _{intro[:50]}_" if intro else "")
                + f"\n{stats_line}\n"
                f"**ENTER FINAL** - **SKIP** - **MENU**"
            ),
            color=theme
        )
    ...
    fill_boss_info_embed(embed, guild, boss, compact=True)
    embed.set_footer(text=level_name)
    return embed
